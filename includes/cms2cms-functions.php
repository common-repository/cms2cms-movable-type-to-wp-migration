...
/**
 * Class CmsPluginFunctionsMovable
 */
class CmsPluginFunctionsMovable
{
    ...
    private $jsConfig = '{
        ...
        "wp_feedback"  : "//wordpress.org/support/plugin/cms2cms-movable-type-to-wp-migration/reviews",
    ...
    {
        $cms2cms_movable_access_login = get_option('cms2cms-movable-login');
        $cms2cms_movable_access_key   = get_option('cms2cms-movable-key');

        return array(
            'email'     => $cms2cms_movable_access_login,
            'accessKey' => $cms2cms_movable_access_key
        );
    }
    ...
    public function isActivated()
    {
        ...
        return ($cms2cms_movable_access_key != false);
    }
    ...
    public function getOptions()
    {
        ...
        $login = get_option('cms2cms-movable-login');
    ...
    public function saveOptions()
    {
        ...
        if ( $key && $login ) {
            ...
            add_option('cms2cms-movable-key', $key);

            delete_option('cms2cms-movable-login');
            add_option('cms2cms-movable-login', $login);

            delete_option('cms2cms-movable-depth');
            add_option('cms2cms-movable-depth', $bridge_depth);
    ...
    {
        return $this->__('Movable Type', 'cms2cms-movable-migration');
    }
    ...
    {
        return $this->__('WordPress', 'cms2cms-movable-migration');
    }
    ...
    {
        return sprintf(
            $this->__('CMS2CMS: Automated %s to %s Migration ', 'cms2cms-movable-migration'),
    ...
    {
        return sprintf(
            $this->__('%s to %s', 'cms2cms-movable-migration'),
            $this->getPluginSourceName(),
            $this->getPluginTargetName()
        );
    }
    ...
    public function logOut()
    {
        if (isset($_REQUEST['_wpnonce'])) {
            ...
            if ($this->verifyFormTempKey($nonce, 'cms2cms_movable_logout')
                && $_POST['cms2cms_movable_logout'] == 1
            ) {
                $this->clearOptions();
            }
        }
