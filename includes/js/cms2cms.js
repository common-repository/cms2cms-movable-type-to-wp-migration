if (typeof jQuery == 'undefined') {
    ...
    jQuery(document).ready(function () {
        if ($('.cms2cms-movable-container').length > 0) {
            ...
            var init = function () {
                ...
                if ($('.active').hasClass('step-sign')) {
                    ...
                    $(document).on('click', '#verifySource_movable', function (event) {
                ...
                $('#auth_submit').on('click', function () {
                    ...
                    $.ajax({
                        ...
                        success: function (data) {
                            ...
                            if (data.key) {
                                saveUser({
                                    action: 'cms2cms_movable_save_options',
                                    accessKey: data.key,
            ...
            var toggleSignIN = function () {
                var cms2cmsBlock = $('#cms2cms_movable_accordeon');
                var signInTabs = cms2cmsBlock.find('a.nav-tab');

                signInTabs.on('click', function (e) {
                    if (!$(this).hasClass('cms2cms-movable-real-link')) {
                        e.preventDefault();
                        var activeClass = 'nav-tab-active';
                        if (!$(this).hasClass(activeClass)) {
                            signInTabs.removeClass(activeClass);
                            $(this).addClass(activeClass);
                            $(this).closest('.cms2cms_movable_accordeon_item').find('form').attr('action', $(this).attr('href'));
                        }
                    }
                });

                var formContent = $('.cms2cms_movable_accordeon_item_register').find('.center-content'),
                    button = $('.login-reg');
                ...
                button.on('click', function () {
                    ...
                    $('.cms2cms-movable-real-link').fadeToggle();
                    button.parent('p.account-register').toggle();
                    button.parent('p.account-login').toggle();
                    $('.cms2cms-movable-error span').parent().fadeOut();
            ...
            var checkDoneTab = function () {
                ...
                if ($('.cms2cms-movable-container').hasClass('cms2cms_movable_is_activated')) {
            ...
            var validateSource = function () {
                ...
                switch (true) {
                    case $('#sourceUrl').val().length == 0:
                        $('.cms2cms-movable-error span').html('Url can`t be empty').parent().fadeIn();
                        break;
                    case !regex.test(sourceUrl):
                        $('.cms2cms-movable-error span').html('Url invalid, please enter url in "http://your_movable_type__website.com/" format').parent().fadeIn();
                        break;
                    case sourceUrl == targetUrl:
                        $('.cms2cms-movable-error span').html('Please, make sure to provide your source website URL (the URL of your current website)').parent().fadeIn();
                        break;
                    default :
                        $('#sourceUrl').val(sourceUrl);
                        ajaxValidate($('#verifySource_movable').closest('form').serialize());
                        return;
                }
